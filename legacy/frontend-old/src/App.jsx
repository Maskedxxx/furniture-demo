...
const VALID_USERS = {
  "demo@alternativa.ru": "demo2026",
  "admin@alternativa.ru": "admin2026",
};
...
const Header = ({ onBack, onLogout, showBack = true }) => (
  <header style={s.header}>
    <div style={s.headerLeft}>
      ...
      <span style={s.headerTitle}>Альтернатива</span>
    </div>
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "#f5f1ec", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "60px 50px", zIndex: 1 }}>
        <div style={{ marginBottom: 48 }}>
          ...
          <h1 style={{ fontSize: 38, fontWeight: 300, color: "#2a2420", letterSpacing: "0.04em", margin: 0 }}>Альтернатива</h1>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 40, zIndex: 1 }}>
        <div style={s.card}>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="demo@alternativa.ru"
          ...
          <div style={{ marginTop: 24, textAlign: "center" }}>
            <div style={{ fontSize: 11, color: "#9a8e84", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 }}>Демо-аккаунт:</div>
            <code style={s.code}>demo@alternativa.ru</code>{" "}
            <code style={s.code}>demo2026</code>
          </div>
        </div>
      </div>
    </div>
...
function ScenarioAPage({ onResult, onBack, onLogout }) {
  ...
  const handleGenerate = () => {
    ...
    setTimeout(() => {
      onResult({
        scenario: "A",
        params: [style, facade, countertop, material].filter(Boolean),
        beforeImage: preview,
      });
    }, 2500);
  };
  ...
  if (generating) {
    return (
      <div style={s.page}><BgPattern /><Header showBack={false} onLogout={onLogout} />
        <main style={{ ...s.main, maxWidth: 500 }}><Spinner text="ИИ генерирует рендер..." sub="Обычно это занимает 20–40 секунд" /></main>
      </div>
    );
  }
...
function ScenarioBPage({ onResult, onBack, onLogout }) {
  ...
  if (generating) {
    return (
      <div style={s.page}><BgPattern /><Header showBack={false} onLogout={onLogout} />
        <main style={{ ...s.main, maxWidth: 500 }}><Spinner text="ИИ заменяет столешницу..." sub="Обычно это занимает 15–30 секунд" /></main>
      </div>
    );
  }
...
function ResultPage({ data, onBack, onLogout }) {
  ...
  const afterImg = PLACEHOLDER_AFTER;
    ...
    <div style={s.page}>
      ...
      <main style={{ ...s.main, maxWidth: 820 }}>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          <button style={s.btnPrimary}>📥 Скачать результат</button>
          <button style={s.btnSecondary}>🔄 Перегенерировать</button>
          <button style={s.btnSecondary}>📤 Отправить в CRM</button>
        </div>
      </main>
    </div>
  );
